...
const AcademicBooks = () => {
  ...
  return (
    <div className="rounded-md my-[20px] bg-slate-100 p-[20px]">
      ...
      {isAcademicBookListLoading && <h4 className="text-center">Loading...</h4>}
      {!isAcademicBookListLoading && academicBookListError && (
        <h4 className="text-center text-red-500">Error</h4>
      )}
      {!isAcademicBookListLoading &&
      !academicBookListError &&
      academicBookListData &&
      academicBookListData.length > 0 ? (
        <>
          <div className="bg-white flex flex-row flex-wrap p-2 rounded-md mt-[20px]">
            {academicBookListData.map((item, index) => (
              <div
                key={index}
